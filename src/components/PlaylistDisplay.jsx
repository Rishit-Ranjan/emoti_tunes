            ...
            <div className="flex-1 overflow-y-auto pb-44 scrollbar-hide">
                <div className={`pt-32 pb-12 px-10 bg-gradient-to-b ${emotion?.gradient || 'from-violet-500/10 to-[#0a0a12]'} to-[#0a0a12]/0 flex items-end space-x-10 relative`}>
                    ...
                    <div className="relative z-10 pb-4">
                        <p className="uppercase text-[10px] font-black tracking-[0.5em] text-cyan-400 mb-4 animate-pulse">Intelligence Verified</p>
                        <h1 className="text-7xl md:text-[8rem] font-black text-white mb-6 tracking-tighter leading-none drop-shadow-2xl uppercase selection:bg-cyan-500">{emotion?.name || 'Vibe'}</h1>
                        <div className="flex items-center space-x-4 text-xs font-black text-white/80">
                            ...
                            <span className="text-white/20 tracking-widest uppercase">• {playlist.length} Sonic Elements</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="px-10 bg-gradient-to-b from-black/20 to-[#0a0a12] pt-8 min-h-screen">
                    <div className="flex items-center space-x-10 mb-12">
                        ...
                        <button onClick={onSave} className="flex items-center space-x-4 bg-white/5 hover:bg-white/10 text-white px-10 py-5 rounded-[2rem] border border-white/10 transition-all font-black uppercase text-[10px] tracking-[0.2em] active:scale-95 group">
                            <svg className="w-6 h-6 text-cyan-400 group-hover:scale-125 transition-transform" fill="currentColor" viewBox="0 0 24 24"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/></svg>
                            <span>Initialize Vault Save</span>
                        </button>
                    </div>

                    <div className="grid grid-cols-[40px_1fr_1fr] md:grid-cols-[40px_4fr_3fr_1fr] gap-8 px-8 py-5 border-b border-white/5 text-[9px] font-black tracking-[0.4em] text-white/20 uppercase mb-8">
                            ...
                            return (
                                <div key={index} onClick={() => setCurrentIndex(index)} className={`group grid grid-cols-[40px_1fr_1fr] md:grid-cols-[40px_4fr_3fr_1fr] gap-8 px-6 py-4 rounded-[2rem] hover:bg-white/5 transition-all cursor-pointer items-center border border-transparent hover:border-white/10 ${isCurrentlyPlaying ? 'bg-white/5 border-white/10 shadow-2xl' : ''}`}>
                                    <div className="text-center font-black">
                                        {isCurrentlyPlaying && isPlaying ? <div className="flex justify-center space-x-1 h-4"><div className="w-1 bg-cyan-400 animate-bounce"></div><div className="w-1 bg-violet-400 animate-bounce delay-150"></div><div className="w-1 bg-cyan-400 animate-bounce delay-300"></div></div> : <span className={`text-[11px] ${isCurrentlyPlaying ? 'text-cyan-400' : 'text-white/20'}`}>{index + 1}</span>}
                                    </div>
                                    <div className="flex items-center min-w-0">
                                        ...
                                        <div className="truncate"><div className={`font-black text-xl truncate tracking-tighter ${isCurrentlyPlaying ? 'text-cyan-400' : 'text-white'}`}>{song.title}</div><div className="text-[10px] font-black text-white/30 truncate uppercase tracking-widest mt-1 group-hover:text-cyan-400/50 transition-colors">{song.artist}</div></div>
                                    </div>
                                    <div className="hidden md:block text-[11px] font-black text-white/20 truncate uppercase tracking-widest italic group-hover:text-white/40 transition-colors">Adaptive Studio Mix</div>
                                    <div className="text-right text-[11px] font-black text-white/20 tabular-nums">3:42</div>
                                </div>
                            );
                        })}
                    </div>
            ...
            <div className="fixed bottom-8 left-8 right-8 md:left-[21.5rem] h-28 bg-[#0a0a12]/95 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] px-12 flex items-center justify-between z-50 shadow-[0_40px_120px_rgba(0,0,0,0.9)] animate-in slide-in-from-bottom-20 duration-1000">
                <div className="flex-[0.35] flex items-center min-w-0">
                    ...
                    <div className="truncate pr-6 text-left">
                        <div className="text-white text-base md:text-xl font-black truncate uppercase tracking-tighter hover:text-cyan-400 cursor-pointer transition-colors leading-none mb-1 md:mb-2">{currentSong?.title}</div>
                        <div className="text-[10px] font-black text-white/20 uppercase tracking-[0.3em] truncate">{currentSong?.artist}</div>
                    </div>
                </div>

                <div className="flex-1 flex flex-col items-center max-w-xl">
                    <div className="flex items-center space-x-12 mb-4">
                        <button onClick={handlePrev} className="text-white/20 hover:text-cyan-400 transition-all active:scale-75 disabled:opacity-5 group" disabled={currentIndex === 0}>
                        ...
                        <button onClick={handleNext} className="text-white/20 hover:text-cyan-400 transition-all active:scale-75 disabled:opacity-5 group" disabled={currentIndex === playlist.length - 1}>
                    ...
                    <div className="w-full flex items-center space-x-6 text-[10px] font-black tracking-[0.2em] text-white/20">
                ...
                <div className="flex-[0.35] hidden md:flex justify-end items-center text-white/20 space-x-8 pr-4">
